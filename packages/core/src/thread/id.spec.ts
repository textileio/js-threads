...
import { ID, Variant, V1 } from './id'

describe('Thread ID', () => {
  it('should be able to create a random ID', async () => {
    const i = ID.fromRandom(Variant.Raw, 16)
  ...
  it('should decode an ID from a base-32 encoded string', async () => {
    const i = ID.fromRandom(Variant.Raw, 32)
  ...
  it('should have a valid variant number', async () => {
    let i = ID.fromRandom(Variant.Raw, 16)
    let v = i.variant()
    expect(v).to.equal(Variant.Raw)
    // console.log(`Variant: ${v}`)

    i = ID.fromRandom(Variant.AccessControlled, 32)
    v = i.variant()
