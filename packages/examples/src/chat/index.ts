...
const program = yargs(process.argv.slice(2))
  .default({ 'debug': true }).alias({ 'd': 'debug' }).describe({ 'debug': 'Output extra debugging info' })
  .default({ 'repo': '.threads/chat-js' }).alias({ 'r': 'repo' }).describe({ 'repo': 'Path to local key store' })
  .default({ 'apiAddr': 'http://127.0.0.1:5008' }).alias({ 'a': 'apiAddr' }).describe({ 'apiAddr': 'Threads service API gRPC proxy bind multiaddress' })
  .argv
...
const resetCmds = () => {
  chat = yargs()
    ...
    .scriptName('')
    .version('0.0.1')
    .command(':address', 'Show the host or active thread addresses.', {},
      async function () {
        if (thread.defined()) {
    ...
    .command(':threads', 'List available threads.', {},
      async function () {
    ...
    .command(':create <name>', 'Create a new thread with a given name.',
    ...
    .command(':enter <name>', 'Enter thread with name.',
      { name: { type: 'string', required: true } },
      async function ({ name }) {
    ...
    .command(':keys', `Show the active thread's keys.`, {},
      async function () {
    ...
    .command(':exit', 'Exit the active thread, or whole program if no thread active.', {},
      function () {
        if (thread.defined()) {
    ...
    .command(':replicator <addr>', 'Add a replicator at address to active thread.',
      { addr: { type: 'string', required: true } },
      async function ({ addr }) {
        if (!thread.defined()) throw new Error('enter thread first')
    ...
    .command([':message [txt]', '*'], 'Send a message to the active thread.', {},
      async function (body) {
        if (!thread.defined()) throw new Error('enter thread first')
...
async function main() {
  ...
  service.subscribe(async (rec?: ThreadRecord, err?: Error) => {
    ...
    console.log(grey(moment(header.time * 1000).local().format('hh:mm:ss A')).padEnd(20) + ' ' + pink(id.slice(-7)) + ' ' + green(body.txt))
    rl.prompt()
  })
