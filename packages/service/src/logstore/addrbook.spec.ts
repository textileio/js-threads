...
describe('AddrBook', () => {
  ...
  describe('Adding', () => {
    ...
    it('add a single address', async () => {
      ...
      await ab.put(id, log, 1000, addr) // 1 hr
      expect(await ab.get(id, log)).to.deep.equal(new Set([addr]))
    })
    ...
    it('adding an existing address with an earlier expiration never reduces the TTL', async () => {
      ...
      await ab.put(id, log, 0)
      expect((await ab.get(id, log)).size).to.equal(0)
    })
  })
  describe('Clearing', () => {
    const id = ThreadID.fromRandom(Variant.Raw, 24)
    ...
    it('clearing out addresses should always work', async () => {
      ...
      expect((await ab.get(id, logs[0])).size).to.equal(0)
      expect((await ab.get(id, logs[1])).size).to.equal(0)
    })
  })
})
